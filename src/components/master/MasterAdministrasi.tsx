import { Plus, Cog, Edit2, Trash2 } from 'lucide-react';
import { useCallback, useEffect, useState } from 'react';
import { supabase } from '../../lib/supabase';

export default function MasterAdministrasi() {
  ...
  useEffect(() => {
    ...
    const fetchAdmin = async () => {
      if (!supabase) return;
      const { data } = await supabase.from('administrations').select('id,code,name,amount').is('deleted_at', null).order('name', { ascending: true });
      if (!data || !active) return;
      setRows((data as { id: string; code?: string | null; name: string; amount?: number | null }[]).map((d) => ({
        id: d.id,
  ...
  }, [showAddModal, generateNextCode]);
  useEffect(() => {
    if (!supabase) return;
    const ch = supabase
      .channel('realtime-administrations')
      .on('postgres_changes', { event: '*', schema: 'public', table: 'administrations' }, async () => {
        if (!supabase) return;
        const { data } = await supabase.from('administrations').select('id,code,name,amount').is('deleted_at', null).order('name', { ascending: true });
        if (!data) return;
        setRows((data as { id: string; code?: string | null; name: string; amount?: number | null }[]).map((d) => ({
  ...
  return (
    <div className="p-8">
      <div className="mb-8 flex items-center justify-between">
        <div>
          <h1 className="text-3xl font-bold text-gray-800">Master Administrasi</h1>
          <p className="text-gray-600 mt-1">Kelola biaya administrasi klinik</p>
        </div>
        <button
        ...
        >
          ...
          <span className="font-medium">Tambah Biaya</span>
        </button>
      </div>
                  ...
                  <td className="py-4 px-6 text-right">
                    <div className="flex justify-end gap-2">
                      ...
                      <button
                        onClick={async () => {
                          if (!supabase) return;
                          await supabase.from('administrations').update({ deleted_at: new Date().toISOString() }).eq('id', row.id);
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white rounded-xl shadow-xl w-full max-w-md mx-4">
            <div className="p-6 border-b border-gray-200">
              <h2 className="text-2xl font-bold text-gray-800">Tambah Biaya Administrasi</h2>
            </div>
              ...
              <button
                onClick={async () => {
                  ...
                  const { data: inserted } = await supabase
                    ...
                    .insert({ code: addCode || null, name, amount: addPrice })
                    .select('id,code,name,amount')
                    .single();
                  if (inserted) {
                    setRows((prev) => [...prev, { id: inserted.id, code: inserted.code || undefined, name: inserted.name, price: Number(inserted.amount ?? 0) }]);
                  }
      ...
      {showEditModal && editId !== null && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white rounded-xl shadow-xl w-full max-w-md mx-4">
            <div className="p-6 border-b border-gray-200">
              <h2 className="text-2xl font-bold text-gray-800">Edit Biaya Administrasi</h2>
            </div>
            <div className="p-6 space-y-4">
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-2">Kode</label>
            ...
            <div className="p-6 border-t border-gray-200 flex gap-3">
              ...
              <button
                onClick={async () => {
                  ...
                  await supabase.from('administrations').update({ name, amount: editPrice }).eq('id', editId);
                  setRows((prev) => prev.map((r) => (r.id === editId ? { ...r, name, price: editPrice } : r)));
                  setShowEditModal(false);
                  setEditId(null);
                }}
                className="flex-1 px-4 py-2 bg-emerald-600 text-white rounded-lg hover:bg-emerald-700 transition-colors font-medium"
